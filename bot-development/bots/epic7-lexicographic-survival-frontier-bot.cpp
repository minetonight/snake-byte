...
static constexpr const char* BOT_LOG_PREFIX = "epic7_lexicographic_survival_frontier_bot_log_";
static constexpr int TURN_BUDGET_MS = 66;
...
static constexpr int APPLE_COMMIT_MIN_BUDGET_MS = 3;
static constexpr int APPLE_COMMIT_MAX_BUDGET_MS = 6;
static constexpr int POST_SCAN_SAFETY_RESERVE_MS = 8;
static constexpr int DEEP_SURVIVAL_GROWTH_DEPTH = 12;
static constexpr int DEEP_SURVIVAL_GROWTH_EXPANSION = 1400;
...
ofstream make_log_stream() {
    ...
    try {
        ...
        try {
            ...
            preferred_dirs.push_back(exe_path.parent_path().parent_path() / "read-logs-here");
        } catch (...) {
        }
        ...
        for (const auto& preferred_dir : preferred_dirs) {
            ...
            ofstream preferred((preferred_dir / log_name).string(), ios::app);
...
unordered_map<uint64_t, bool> g_future_growth_cache_by_hash;
...
inline bool can_snake_enter_cell_on_next_step(const vector<int16_t>& grid, const Snake& s, int pos) {
    if (pos < 0 || pos >= grid_size) return false;
    int16_t cell = grid[pos];
...
struct GameState {
    ...
    int count_safe_followups(const Snake& s) const {
        ...
        for (int a = 0; a < 4; ++a) {
            ...
            if (nx < 0 || nx >= max_width || ny < 0 || ny >= max_height) continue;
            int n_pos = ny * max_width + nx;
            if (!can_snake_enter_cell_on_next_step(grid, s, n_pos)) continue;
...
static int first_legal_action_basic(const GameState& state, const Snake& s) {
    ...
    for (int a = 0; a < 4; ++a) {
        ...
        if (nx < 0 || nx >= max_width || ny < 0 || ny >= max_height) continue;
        int n_pos = ny * max_width + nx;
        if (!can_snake_enter_cell_on_next_step(state.grid, s, n_pos)) continue;
        return a;
    }
...
static bool is_action_locally_legal(const GameState& state, const Snake& s, int action) {
    ...
    int n_pos = ny * max_width + nx;
    return can_snake_enter_cell_on_next_step(state.grid, s, n_pos);
}
...
static int choose_safe_action_for_target(const GameState& state, const Snake& s, int target_pos) {
    ...
    bool require_full_body_chamber = target_pos >= 0
        && target_pos < grid_size
        && state.my_snakes.size() <= 1
        && count_powerups_on_grid(state) <= 1
        && state.grid[target_pos] == CELL_POWERUP;
    ...
    for (int action = 0; action < 4; ++action) {
        ...
        if (next_self == nullptr || !next_self->is_alive || next_self->length < s.length) continue;
        int next_head = next_self->body[next_self->head_idx];
        if (require_full_body_chamber) {
            int chamber_need = max(next_self->length, s.length);
            if (!isolated.survives_flood_fill(next_head, chamber_need)) continue;
        }
...
static DirectAppleHint find_direct_apple_hint(const GameState& state, const Snake& s, const unordered_set<int>& forbidden_targets = {}) {
    ...
    for (int pos = 0; pos < grid_size; ++pos) {
        ...
        if (!can_snake_enter_cell_on_next_step(state.grid, s, next_pos)) continue;
...
struct DeepSurvivalSearchResult {
    int action = -1;
    int best_depth = -1;
    int best_min_length = -1;
    int best_final_length = -1;
    int best_followups = -1;
    int best_outside_distance = INT_MAX;
    bool found_cycle = false;
    bool future_growth = false;
    int expanded = 0;
    int explored_states = 0;
    int depth_limit = 0;
...
struct DeepSurvivalVisitedState {
    int shallowest_depth = INT_MAX;
    int best_min_length = -1;
};

struct DeepSurvivalScore {
    int survival_depth = -1;
    int min_length = -1;
    int final_length = -1;
    int followups = -1;
    int outside_distance = INT_MAX;
    bool future_growth = false;
    bool found_cycle = false;
};

static bool is_better_deep_survival_score(const DeepSurvivalScore& candidate, const DeepSurvivalScore& incumbent) {
    if (candidate.survival_depth != incumbent.survival_depth) return candidate.survival_depth > incumbent.survival_depth;
    if (candidate.min_length != incumbent.min_length) return candidate.min_length > incumbent.min_length;
    if (candidate.final_length != incumbent.final_length) return candidate.final_length > incumbent.final_length;
    if (candidate.future_growth != incumbent.future_growth) return candidate.future_growth;
    if (candidate.found_cycle != incumbent.found_cycle) return candidate.found_cycle;
    if (candidate.followups != incumbent.followups) return candidate.followups > incumbent.followups;
    if (candidate.outside_distance != incumbent.outside_distance) return candidate.outside_distance < incumbent.outside_distance;
    return false;
}
...
static int max_reachable_survival_depth(const GameState& state, int snake_id, int depth_limit, int expansion_limit) {
    ...
    while (!q.empty() && !out_of_time() && expanded < expansion_limit) {
        ...
        for (int action = 0; action < 4; ++action) {
            ...
            int n_pos = ny * max_width + nx;
            if (!can_snake_enter_cell_on_next_step(node.state.grid, *cur_self, n_pos)) continue;
...
static DeepSurvivalSearchResult find_deep_survival_action(const GameState& state, int snake_id, int depth_limit, int expansion_limit) {
    ...
    result.expansion_limit = expansion_limit;
    ...
    deque<SearchNode> nodes;
    nodes.push_back({start_state, -1, 0, start_self->body[start_self->head_idx]});
    vector<int> node_min_lengths;
    node_min_lengths.push_back(start_self->length);

    queue<int> q;
    q.push(0);

    unordered_map<uint64_t, DeepSurvivalVisitedState> visited;
    visited.reserve(1024);
    visited[encode_single_snake_plan_hash(start_state, *start_self)] = {0, start_self->length};
    int expanded = 0;
    int dx[] = {0, 0, -1, 1};
    int dy[] = {-1, 1, 0, 0};

    auto consider = [&](int first_action, const DeepSurvivalScore& score) {
        if (first_action == -1) return;
        DeepSurvivalScore incumbent{result.best_depth, result.best_min_length, result.best_final_length,
                                    result.best_followups, result.best_outside_distance,
                                    result.future_growth, result.found_cycle};
        if (result.action == -1 || is_better_deep_survival_score(score, incumbent)) {
            result.action = first_action;
            result.best_depth = score.survival_depth;
            result.best_min_length = score.min_length;
            result.best_final_length = score.final_length;
            result.best_followups = score.followups;
            result.best_outside_distance = score.outside_distance;
            result.found_cycle = score.found_cycle;
            result.future_growth = score.future_growth;
        }
    };
    ...
    while (!q.empty() && !out_of_time() && expanded < expansion_limit) {
        ...
        const SearchNode& node = nodes[idx];
        const Snake* cur_self = node.state.find_my_snake_by_id(snake_id);
    if (cur_self == nullptr || !cur_self->is_alive || cur_self->length < 3) continue;
        ...
        for (int action = 0; action < 4; ++action) {
            ...
            const Snake* next_self = next_state.find_my_snake_by_id(snake_id);
            if (next_self == nullptr || !next_self->is_alive || next_self->length < 3) continue;

            int first_action = (node.first_action == -1) ? action : node.first_action;
            int next_depth = node.depth + 1;
            int next_min_length = min(node_min_lengths[idx], next_self->length);
            int followups = next_state.count_safe_followups(*next_self);

            int next_head = next_self->body[next_self->head_idx];
            uint64_t h = encode_single_snake_plan_hash(next_state, *next_self);
            auto seen_it = visited.find(h);
            bool found_cycle = seen_it != visited.end();
            bool future_growth = false;
            if (count_powerups_on_grid(next_state) > 0 && !out_of_time()
                && (found_cycle || next_depth >= max(4, depth_limit - 2))) {
                auto fg_it = g_future_growth_cache_by_hash.find(h);
                if (fg_it != g_future_growth_cache_by_hash.end()) {
                    future_growth = fg_it->second;
                } else {
                    if (g_future_growth_cache_by_hash.size() > 20000) {
                        g_future_growth_cache_by_hash.clear();
                    }
                    future_growth = has_reachable_future_growth(next_state, snake_id,
                        DEEP_SURVIVAL_GROWTH_DEPTH, DEEP_SURVIVAL_GROWTH_EXPANSION);
                    g_future_growth_cache_by_hash[h] = future_growth;
                }
            }

            DeepSurvivalScore score{next_depth, next_min_length, next_self->length, followups,
                                    outside_world_distance(next_head), future_growth, found_cycle};
            consider(first_action, score);

            if (seen_it != visited.end()
                && (seen_it->second.best_min_length > next_min_length
                    || (seen_it->second.best_min_length == next_min_length
                        && seen_it->second.shallowest_depth <= next_depth))) {
                continue;
            }
            visited[h] = {next_depth, next_min_length};

            nodes.push_back({next_state, first_action, next_depth, next_head});
            node_min_lengths.push_back(next_min_length);
            q.push(static_cast<int>(nodes.size()) - 1);
        }
    }

    result.expanded = expanded;
    result.explored_states = static_cast<int>(nodes.size());
...
static void debug_log_fixed_left_apple_route(const GameState& state, int snake_id, const string& route) {
    ...
    for (size_t i = 0; i < route.size(); ++i) {
        ...
        int n_pos = ny * max_width + nx;
        if (!can_snake_enter_cell_on_next_step(cur_state.grid, *self, n_pos)) {
            mylog << "DEBUG_ROUTE_FAIL turn=" << g_turn_counter
                  << " snake=" << snake_id
                  << " step=" << (i + 1)
                  << " action=" << route[i]
    ...
    while (1) {
        ...
        for (size_t s_idx = 0; s_idx < state.my_snakes.size(); ++s_idx) {
            ...
            if (!solo_frontier_mode && scan.found_apple_progress && scan.apple_progress_target != -1 && scan.apple_progress_dist <= 2) {
                ...
                int desired_goal_dist = shortest_path_distance_walls_only(state, head_pos, desired_goal);
                if (desired_goal_dist != INT_MAX && remaining_powerups <= 2 && desired_goal_dist <= 8) {
                    apple_commit_target = desired_goal;
                    apple_commit_depth = min(14, desired_goal_dist + 6);
                }
            }
            if (apple_commit_target != -1) {
                int remaining_ms_after_scan = max(0, TURN_BUDGET_MS - elapsed_turn_ms());
                int apple_commit_budget_ms = min(APPLE_COMMIT_MAX_BUDGET_MS,
                    max(0, remaining_ms_after_scan - POST_SCAN_SAFETY_RESERVE_MS));
                if (apple_commit_budget_ms >= APPLE_COMMIT_MIN_BUDGET_MS) {
                    PhaseTimingBudget apple_commit_phase_budget;
                    apple_commit_phase_budget.deep_scan_budget_ms = apple_commit_budget_ms;
                    apple_commit_phase_budget.followthrough_budget_ms = apple_commit_budget_ms;
                    ScopedSearchPhase apple_commit_phase(&apple_commit_phase_budget, SearchPhase::DeepScan);
            ...
            bool suppress_unvalidated_solo_apple_goal = false;
            if (solo_frontier_mode && desired_goal != -1 && state.grid[desired_goal] == CELL_POWERUP
                && apple_commit_target == desired_goal && !apple_commit.found) {
                suppress_unvalidated_solo_apple_goal = true;
                clear_apple_goal(s.id);
                PersistentGoal center_goal = get_or_refresh_center_goal(state, s, reserved_center_targets);
                desired_goal = center_goal.target_pos;
            }
            if (solo_frontier_mode && !suppress_unvalidated_solo_apple_goal
                && desired_goal != -1 && state.grid[desired_goal] == CELL_POWERUP
                && remaining_powerups <= 2
                && !scan.found_apple && !scan.found_apple_progress
                && (!scan.found_goal_progress || scan.goal_dist >= 6)) {
                suppress_unvalidated_solo_apple_goal = true;
                clear_apple_goal(s.id);
                PersistentGoal center_goal = get_or_refresh_center_goal(state, s, reserved_center_targets);
                desired_goal = center_goal.target_pos;
            ...
            DeepSurvivalSearchResult deep_survival;
            bool chosen_by_safe_target_picker = false;
            ...
            bool confined_survival_mode = scan.expanded <= 400
                && (!scan.found_apple || scan.apple_depth >= 6 || scan.apple_followups <= 1);
            bool solo_unproven_apple_goal = solo_frontier_mode
                && desired_goal != -1
                && state.grid[desired_goal] == CELL_POWERUP
                && !apple_commit.found
                && !scan.found_apple
                && !scan.found_apple_progress;
            bool sparse_solo_distant_apple_progress = sparse_solo_apple_field
                && scan.found_apple_progress
                && !apple_commit.found
                && scan.apple_progress_dist >= 5;
            ...
            bool deep_survival_has_frontier_alternative = scan.found_apple || scan.found_apple_progress || scan.found_goal_progress;
            if (!solo_frontier_mode && apple_commit.found) {
                ...
                deep_survival = find_deep_survival_action(state, s.id, 18, 1800);
                bool deep_survival_preserves_length = deep_survival.best_min_length >= s.length;
                bool deep_survival_should_override_frontier = !deep_survival_has_frontier_alternative
                    || deep_survival.future_growth;
                if (deep_survival.action != -1 && deep_survival_preserves_length && deep_survival_should_override_frontier) {
                    chosen_action = deep_survival.action;
                    chosen_target = -1;
                    clear_apple_goal(s.id);
                    mode = deep_survival.future_growth ? "confined_emerge"
                         : (deep_survival.found_cycle ? "confined_cycle" : "confined_survival");
                }
                ...
                mode = "reachable_apple";
                      ...
                      && !sparse_solo_distant_apple_progress
                ...
                if (progress_action != -1) {
                    ...
                    mode = "apple_progress";
                       ...
                       && !suppress_near_unvalidated_apple_goal && !suppress_unvalidated_solo_apple_goal
                       && !solo_unproven_apple_goal) {
            ...
            if (chosen_action == -1) {
                ...
                chosen_by_safe_target_picker = (chosen_action != -1);
            ...
            if (chosen_action != -1 && !chosen_by_safe_target_picker
                && (is_action_immediately_unsafe_against_stronger_snake(state, s, chosen_action) || should_stabilize_choice)) {
            ...
            mylog << "FRONTIER_DECISION turn=" << counter
                  ...
                  << " deep_depth=" << deep_survival.best_depth
                  << " deep_min_length=" << deep_survival.best_min_length
                  << " deep_final_length=" << deep_survival.best_final_length
                  << " deep_followups=" << deep_survival.best_followups
                  << " deep_cycle=" << (deep_survival.found_cycle ? 1 : 0)
                  << " deep_future_growth=" << (deep_survival.future_growth ? 1 : 0)
                  << " deep_expanded=" << deep_survival.expanded
                  << " deep_explored=" << deep_survival.explored_states
                  << " deep_limit=" << deep_survival.depth_limit
